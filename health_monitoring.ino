...
#define DHTPIN 18       // DHT11 sensor pin
#define DHTTYPE DHT11   // Change this to your DHT sensor type (DHT11, DHT22, etc.)

float temperature, humidity, BPM, SpO2, bodytemperature;

const char* ssid = "Redmi Note11T 5G"; // Enter SSID here
const char* password = "redmi123";     // Enter Password here

DHT dht(DHTPIN, DHTTYPE);
PulseOximeter pox;
uint32_t tsLastReport = 0;
OneWire oneWire(5); // DS18B20 sensor pin
...
const unsigned long REPORTING_PERIOD_MS = 1000; // Define your reporting period in milliseconds
...
void setup() {
  ...
  int wifiTimeout = 30; // Set a timeout for Wi-Fi connection attempts (in seconds)
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    while (1) {
      delay(1000); // If Wi-Fi connection fails, stop here
    }
  }
  ...
  if (!pox.begin()) {
    ...
    while (1) {
      delay(1000); // If pulse oximeter initialization fails, stop here
    }
  } else {
